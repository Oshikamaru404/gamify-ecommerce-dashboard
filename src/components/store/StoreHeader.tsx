...
const StoreHeader: React.FC = () => {
  ...
  return <header className="sticky top-0 z-40 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-16 items-center justify-between">
        ...
        <nav className="hidden md:flex md:items-center md:gap-6">
          ...
          <form onSubmit={handleSearch} className="relative w-56">
            <Input type="search" placeholder="Search games..." className="h-9 pr-10" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        ...
        <div className="flex items-center gap-2">
          <Link to="/cart" className="relative">
            <Button variant="ghost" size="icon" className="h-9 w-9">
              ...
              {cartItemsCount > 0 && <span className="absolute -right-2 -top-2 flex h-5 w-5 items-center justify-center rounded-full bg-primary text-xs text-primary-foreground">
                  {cartItemsCount}
                </span>}
            </Button>
          </Link>
      ...
      {/* Mobile Menu */}
      <div className={cn("absolute left-0 right-0 top-16 z-50 border-b bg-background pb-4 pt-2 md:hidden", isMenuOpen ? "block" : "hidden")}>
        <form onSubmit={handleSearch} className="mx-4 mb-4 flex">
          <Input type="search" placeholder="Search games..." className="flex-1" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        ...
        <nav className="flex flex-col space-y-4 px-4">
          ...
          <Link to="/admin" className="py-2 text-sm font-medium" onClick={() => setIsMenuOpen(false)}>
            Admin
          </Link>
        </nav>
      </div>
