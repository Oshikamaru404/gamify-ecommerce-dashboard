...
import { ArrowLeft, Check, Server, Settings, Shield, Star } from 'lucide-react';
        ...
        {options.map((option, idx) => (
          <Card key={idx} className="p-4 border-2 border-gray-100 hover:border-[#8f35e5]/30 transition-all duration-300">
            <div className="text-center">
              ...
              <div className="text-sm text-gray-600 mb-3">Manage {option.credits} subscription slots</div>
...
const IPTVDetail = () => {
  ...
  const handleBuyNow = (credits: number, price: number) => {
    if (!pkg) return;
    setSelectedPackage({ id: pkg.id, name: packageName, category: 'panel-iptv', description: packageDescription, icon_url: pkg.icon_url, price, duration: credits });
  ...
  if (!pkg) {
    return (
      <StoreLayout><div className="bg-gradient-to-br from-gray-50 via-white to-gray-50 min-h-screen"><div className="container py-16 text-center">
        ...
        <Button onClick={() => navigate('/iptv-panel')} className="bg-[#8f35e5] hover:bg-[#7c2fd4]"><ArrowLeft className="mr-2 h-4 w-4" />Back to IPTV Panel</Button>
      </div></div></StoreLayout>
    );
  }
    ...
    <StoreLayout>
      <div className="bg-gradient-to-br from-gray-50 via-white to-gray-50 min-h-screen">
        <div className="container py-16">
          <Button variant="ghost" onClick={() => navigate('/iptv-panel')} className="mb-8 text-gray-600 hover:text-gray-900">
            <ArrowLeft className="mr-2 h-4 w-4" />Back to IPTV Panel
          </Button>
            ...
            <div className="bg-gradient-to-r from-[#8f35e5] to-[#7c2fd4] text-white p-8">
              <div className="flex items-center gap-6">
                <div className="w-20 h-20 flex items-center justify-center">
                  {pkg.icon_url ? <img src={pkg.icon_url} alt={packageName} className="w-20 h-20 rounded-xl object-cover shadow-lg"
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-4">
                    <Badge className="bg-white/20 text-white"><Star className="w-4 h-4 mr-1 fill-current" />Premium IPTV Panel</Badge>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            <div className="lg:col-span-2 space-y-8">
              <Card className="p-6">
                <h2 className="text-2xl font-bold mb-6 flex items-center"><Server className="mr-3 h-6 w-6 text-[#8f35e5]" />Package Features</h2>
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  )) : ['Premium Quality', 'Fast Activation', 'Guaranteed Reliability', '24/7 Support'].map((f, i) => (
                    <div key={i} className="flex items-start">
                      <div className="flex-shrink-0 w-6 h-6 bg-[#8f35e5]/10 rounded-full flex items-center justify-center mt-1 mr-3"><Check className="w-4 h-4 text-[#8f35e5]" /></div>
              ...
              <Card className="p-6">
                <h2 className="text-2xl font-bold mb-6 flex items-center"><Settings className="mr-3 h-6 w-6 text-[#8f35e5]" />Service Information</h2>
                <div className="space-y-4">
                  <div className="bg-[#8f35e5]/5 border border-[#8f35e5]/20 rounded-lg p-4"><h3 className="font-semibold text-[#8f35e5] mb-2">Credit-Based System</h3><p className="text-gray-700">Purchase credits to manage your IPTV subscriptions efficiently.</p></div>
                  <div className="bg-[#8f35e5]/5 border border-[#8f35e5]/20 rounded-lg p-4"><h3 className="font-semibold text-[#8f35e5] mb-2">Professional Management</h3><p className="text-gray-700">Advanced tools for IPTV service management and administration.</p></div>
                </div>
              </Card>
