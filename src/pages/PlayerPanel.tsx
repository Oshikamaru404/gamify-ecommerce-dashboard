...
import { Monitor, Settings, Crown, ArrowRight } from 'lucide-react';
...
const PlayerPackageCards = ({ pkg, onBuyNow }: { pkg: any; onBuyNow: (pkg: any, credits: number, price: number) => void }) => {
                ...
                <div className="text-center">
                  ...
                  <div className="space-y-2">
                    ...
                    <Button asChild variant="outline" className="w-full border-[#8f35e5] text-[#8f35e5] hover:bg-[#8f35e5] hover:text-white">
                      <Link to={`/player-panel/${generateProductSlug(pkg.name)}`}>View Details<ArrowRight className="ml-2 h-4 w-4" /></Link>
                    </Button>
                  </div>
                </div>
              </Card>
      ...
      <div className="bg-gradient-to-br from-gray-50 via-white to-gray-50 min-h-screen">
        <div className="container py-16">
          <section className="mb-20 text-center">
            <h1 className="mb-6 text-5xl font-extrabold tracking-tight text-gray-900 md:text-6xl">Player Panel Credits</h1>
            <p className="mx-auto mb-10 max-w-3xl text-xl text-gray-600">Purchase credits for streaming applications and player activation</p>
          </section>

          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 mb-12">
            <Card className="p-8 text-center hover:shadow-xl transition-all">
              <Monitor className="h-16 w-16 text-[#8f35e5] mx-auto mb-4" /><h3 className="text-xl font-bold mb-3">Player Applications</h3><p className="text-gray-600">Activate streaming apps and players with credit system</p>
            </Card>
            <Card className="p-8 text-center hover:shadow-xl transition-all">
              <Settings className="h-16 w-16 text-[#8f35e5] mx-auto mb-4" /><h3 className="text-xl font-bold mb-3">Easy Management</h3><p className="text-gray-600">Simple credit-based activation system</p>
            </Card>
            <Card className="p-8 text-center hover:shadow-xl transition-all">
              <Crown className="h-16 w-16 text-[#8f35e5] mx-auto mb-4" /><h3 className="text-xl font-bold mb-3">Lifetime Options</h3><p className="text-gray-600">2 credits = lifetime activation for supported apps</p>
            </Card>
          </div>

          <section className="space-y-16">
            {playerPackages.length > 0 ? (
              playerPackages.map(pkg => <PlayerPackageCards key={pkg.id} pkg={pkg} onBuyNow={handleBuyNow} />)
            ) : (
              <div className="text-center py-16">
                <h3 className="text-2xl font-bold text-gray-900 mb-4">No Player Packages Available</h3>
                <p className="text-gray-600">Player packages are currently being updated.</p>
              </div>
            )}
          </section>
          ...
          <div className="text-center mt-16">
            <div className="bg-[#8f35e5]/5 rounded-2xl p-8">
              ...
              <p className="text-lg text-gray-700 max-w-3xl mx-auto">Contact our support team for assistance with credit purchases and app activation.</p>
            </div>
          </div>
        </div>
